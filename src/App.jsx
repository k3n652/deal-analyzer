...
export default function App() {
  ...
  useEffect(() => { loadDeals().then((d) => { setDeals(d); setLoading(false); }); }, []);
  ...
  const s = { // shared input style
    bg: { background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, color: C.text, fontSize: 15, padding: "10px 12px", width: "100%", outline: "none", boxSizing: "border-box" },
    label: { color: C.muted, fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 5, display: "block" },
  };

  const Field = ({ k, label, placeholder, prefix = true, textarea = false }) => (
    <div>
      <label style={s.label}>{label}</label>
      <div style={{ position: "relative" }}>
        {prefix && !textarea && <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.muted }}>$</span>}
        {textarea
          ? <textarea rows={3} placeholder={placeholder} value={inputs[k]} onChange={set(k)}
              style={{ ...s.bg, paddingLeft: 12, resize: "vertical", lineHeight: 1.5 }} />
          : <input type={prefix ? "number" : "text"} placeholder={placeholder} value={inputs[k]} onChange={set(k)}
              style={{ ...s.bg, paddingLeft: prefix ? 24 : 12 }} />}
      </div>
    </div>
  );
  ...
  const Btn = ({ onClick, disabled, color, textColor, children, style = {} }) => (
        ...
        {tab === "analyze" && (
          <>
            ...
            <div style={{ display: "grid", gap: 14, marginBottom: 20 }}>
              <Field k="address" label="Property Address / Label" placeholder="e.g. 123 Main St" prefix={false} />
              <Field k="arv" label="After Repair Value (ARV)" placeholder="e.g. 200000" />
              <Field k="repairs" label="Estimated Repairs" placeholder="e.g. 30000" />
              <Field k="askingPrice" label="Seller's Asking Price" placeholder="e.g. 100000" />
              <Field k="assignmentFee" label="Your Assignment Fee" placeholder="default $5,000" />
              <Field k="closingCosts" label="Closing Costs (est.)" placeholder="default $3,000" />
              <Field k="notes" label="Notes" placeholder="Motivated seller, needs roof, off-market..." prefix={false} textarea />
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              ...
              <Btn onClick={() => {
                ...
                exportPDF({
                  ...
                  arv, repairs, askingPrice, assignmentFee, closingCosts,
                  mao70, maoWholesaler, spread, profitMargin, isDeal, notes: inputs.notes,
                });
